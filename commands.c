...
int Cmd_read(int argc, char *argv[])
{
    ...
    {
        //Si los parametros no son suficientes o son demasiados, muestro la ayuda
        UARTprintf(" read [color]\n");
    }
    else
    {

        if (0==strncmp( argv[1], "color",5))
        {
            uint16_t r, g, b, c;
            char cadena[32];
            int16_t longitud;

            if (  !SF_APDS9960_readAmbientLight(&c) ||
...
void vUARTTask( void *pvParameters )
{
	...
	//: Esto se ha puesto en esta tarea, pero podria moverse a otra...
	// Inicializa la biblioteca que gestiona el bus I2C.
	if(I2C_IF_Open(I2C_MASTER_MODE_FST) < 0)
	{
	    while (1);
	}
	vTaskDelay(10); //Espera un poco
	//Inicializa el sensor y configura los parametros por defecto
	SF_APDS9960_init();
	SF_APDS9960_enableLightSensor(false); //Habilita el sensado de luz
	// XXX. A partir de aqui, el sensor queda inicializado...
